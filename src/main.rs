    ...
    for gx in min_gx..=max_gx {
        for gy in min_gy..=max_gy {
            for gz in min_gz..=max_gz {
                let center = Vec3::new(
                    gx as f32 * voxel_size + ext,
                    gy as f32 * voxel_size + ext,
                    gz as f32 * voxel_size + ext,
                );
                ...
                // SAT Test 1: Voxel AABB vs Triangle AABB
                if v0.min(v1).min(v2).max_element() > ext || v0.max(v1).max(v2).min_element() < -ext {
                    continue;
                }

                // SAT Test 2: Triangle Plane vs Voxel AABB
                ...
                if test_axis(Vec3::new(0.0, -f0.z, f0.y)) || test_axis(Vec3::new(0.0, -f1.z, f1.y)) || test_axis(Vec3::new(0.0, -f2.z, f2.y)) ||
                   test_axis(Vec3::new(f0.z, 0.0, -f0.x)) || test_axis(Vec3::new(f1.z, 0.0, -f1.x)) || test_axis(Vec3::new(f2.z, 0.0, -f2.x)) ||
                   test_axis(Vec3::new(-f0.y, f0.x, 0.0)) || test_axis(Vec3::new(-f1.y, f1.x, 0.0)) || test_axis(Vec3::new(-f2.y, f2.x, 0.0)) {
                    continue;
                }
...
fn run_background_voxelization(
    ...
    let batch_size = 2048;
    let sampled_batches: Vec<Vec<(u64, [f32; 3])>> = normalized_triangles
        .par_chunks(batch_size)
        .map(|chunk| {
            let mut local_out = Vec::with_capacity(chunk.len() * 2);
            for &(pts, color) in chunk {
                sample_triangle(pts, color, voxel_size, &mut local_out);
            }
                ...
                for x in 0..32 {
                    for z in 0..32 {
                        for y in 0..4 {
                            ...
                            octree.insert_cube(x * 8, y * 8, z * 8, depth_limit, mat, false);
                        }
                    }
                }
            }
            ...
            for x in 0..32 {
                for y in 0..32 {
                    for z in 0..32 {
                        ...
                        if density > 0.12 {
                            octree.insert_cube(x * 8, y * 8, z * 8, depth_limit, 2, false);
                        }
                    }
                }
            }
...
fn emit_quad(
    ...
    for (i, &pos) in corners.iter().enumerate() {
        vertices.push(Vertex { position: pos, normal, color, uv: QUAD_UVS[i] });
    }
    indices.extend_from_slice(&[start, start + 1, start + 2, start, start + 2, start + 3]);
}

fn is_face_occluded(
    octree: &Octree, gx: u32, gy: u32, gz: u32, size: u32, axis: usize, positive: bool,
) -> bool {
...
fn mesh_octree_node(
    octree: &Octree, node_idx: usize, gx: u32, gy: u32, gz: u32, size: u32, chunk_pos: ChunkPos, palette: &[[f32; 3]], vertices: &mut Vec<Vertex>, indices: &mut Vec<u32>,
) {
    ...
    let color = if mat_id >= 1 && (mat_id as usize) <= palette.len() { palette[(mat_id - 1) as usize] } else { [0.5, 0.5, 0.5] };
...
pub fn update_chunk_buffers(chunk: &mut RenderChunk, payload: &MeshPayload, device: &wgpu::Device, queue: &wgpu::Queue) {
    ...
    if v_bytes.len() > chunk.vertex_capacity {
        ...
        chunk.vertex_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: None, size: chunk.vertex_capacity as u64, usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST, mapped_at_creation: false,
        });
    }
    ...
    if i_bytes.len() > chunk.index_capacity {
        ...
        chunk.index_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: None, size: chunk.index_capacity as u64, usage: wgpu::BufferUsages::INDEX | wgpu::BufferUsages::COPY_DST, mapped_at_creation: false,
        });
    }
...
impl ChunkManager {
    ...
    fn modify_cube(&mut self, pos: Vec3, material: u16, size: f32, device: &wgpu::Device, queue: &wgpu::Queue) {
        let edit = CubeEdit { pos: [pos.x, pos.y, pos.z], size, material };
    ...
    fn update(&mut self, player_pos: Vec3, device: &wgpu::Device) {
        ...
        for x in -self.render_distance..=self.render_distance {
            for z in -self.render_distance..=self.render_distance {
                let dist_sq = x*x + z*z;
                let lod_level = if dist_sq < 9 { 0 } else if dist_sq < 25 { 1 } else { 2 };
                ...
                if !self.loaded_chunks.contains_key(&pos) && !self.loading_chunks.contains(&pos) {
                    ...
                    rayon::spawn(move || {
                        let octree = generate_octree(pos, wtype, seed, &edits, lod_level);
        ...
        while let Ok((pos, octree, payload)) = self.rx.try_recv() {
            ...
            self.loaded_chunks.insert(pos, RenderChunk {
                octree, vertex_buffer, index_buffer, num_indices: payload.indices.len() as u32,
                vertex_capacity: v_bytes.len(), index_capacity: i_bytes.len(),
            });
        }
...
#[derive(Default)]
struct InputState { 
    forward: bool, backward: bool, left: bool, right: bool, up: bool, down: bool, 
    action_add: bool, action_remove: bool, action_pick: bool,
}
...
const PRESET_SWATCHES: [[f32; 3]; 10] = [
    [0.95, 0.95, 0.95], [0.10, 0.10, 0.12], [0.90, 0.20, 0.20], [0.95, 0.50, 0.15], [0.95, 0.85, 0.15],
    [0.20, 0.75, 0.25], [0.15, 0.80, 0.85], [0.20, 0.45, 0.90], [0.65, 0.25, 0.85], [0.55, 0.35, 0.20],
];
...
struct GizmoAxis {
    dir: Vec3, name: &'static str, color: [f32; 4], yaw: f32, pitch: f32, is_positive: bool,
}
...
fn build_ui_vertices(
    ...
    for (i, &rgb) in hotbar_colors.iter().enumerate() {
        ...
        if selected_slot == i { add_quad(&mut verts, x0 - 0.006, y_bottom - 0.006, x1 + 0.006, y_top + 0.006, [1.0, 0.9, 0.1, 1.0]); } 
        else { add_quad(&mut verts, x0 - 0.003, y_bottom - 0.003, x1 + 0.003, y_top + 0.003, [0.15, 0.16, 0.20, 0.9]); }
    ...
    let size_str = if edit_size < 0.001 { format!("RES: {:.1e}", edit_size) } 
               else if edit_size < 1.0 { format!("RES: 1/{} ({:.4})", (1.0 / edit_size).round() as u32, edit_size) } 
               else { format!("RES: {:.0}X{:.0}", edit_size, edit_size) };
    ...
    match active_menu {
        ActiveMenu::None => {
            ...
            if cursor_free { draw_text(&mut verts, "CURSOR FREE  |  DRAG & DROP .GLB OR OPEN ESC MENU", -0.96, 0.86, 1.0, aspect, [0.95, 0.45, 0.2, 0.95]); } 
            else if let Some(tpos) = target_pos { draw_text(&mut verts, &format!("AIM: [{:.3}, {:.3}, {:.3}] (SNAP)", tpos[0], tpos[1], tpos[2]), -0.96, 0.86, 1.0, aspect, [0.3, 0.9, 0.9, 0.9]); } 
            else { draw_text(&mut verts, "AIM: [UNBOUNDED VOID - PLACES IN FRONT]", -0.96, 0.86, 1.0, aspect, [0.6, 0.7, 0.8, 0.8]); }
        ...
        ActiveMenu::Edit => {
            ...
            for (i, &rgb) in hotbar_colors.iter().enumerate() {
                ...
                let num_str = if i == 9 { "0".to_string() } else { format!("{}", i + 1) };
                draw_text_centered(&mut verts, &num_str, (sx0 + sx1) / 2.0, 0.542, 1.0, aspect, [0.8, 0.8, 0.8, 0.9]);
            }
            ...
            add_quad(&mut verts, 0.22, -0.10, 0.40, -0.02, [0.35, 0.40, 0.55, 1.0]); draw_text_centered(&mut verts, "* 2 (R)", 0.31, -0.06, 1.1, aspect, [1.0, 1.0, 1.0, 1.0]);
            add_quad(&mut verts, -0.22, -0.25, 0.22, -0.17, [0.20, 0.50, 0.30, 1.0]); draw_text_centered(&mut verts, "DONE (PRESS E)", 0.0, -0.21, 1.1, aspect, [1.0, 1.0, 1.0, 1.0]);
        }
        ActiveMenu::Pause => {
            add_quad(&mut verts, -1.0, -1.0, 1.0, 1.0, [0.03, 0.04, 0.06, 0.80]);
            add_quad(&mut verts, -0.386, -0.726, 0.386, 0.726, [0.45, 0.45, 0.50, 1.0]);
            add_quad(&mut verts, -0.38, -0.72, 0.38, 0.72, [0.12, 0.13, 0.17, 0.98]);
            ...
            add_quad(&mut verts, -0.30, 0.36, 0.30, 0.44, [0.25, 0.35, 0.55, 1.0]); draw_text_centered(&mut verts, if play_mode == PlayMode::Flying { "PLAY MODE: FLYING" } else { "PLAY MODE: REAL" }, 0.0, 0.40, 1.1, aspect, [1.0, 1.0, 1.0, 1.0]);
            add_quad(&mut verts, -0.30, 0.26, 0.30, 0.34, [0.22, 0.40, 0.55, 1.0]); draw_text_centered(&mut verts, if is_ortho { "VIEW: ORTHOGRAPHIC" } else { "VIEW: PERSPECTIVE" }, 0.0, 0.30, 1.1, aspect, [1.0, 1.0, 1.0, 1.0]);
            add_quad(&mut verts, -0.30, 0.16, 0.30, 0.24, [0.35, 0.25, 0.50, 1.0]); draw_text_centered(&mut verts, &format!("WORLD: {}", world_type.name()), 0.0, 0.20, 1.1, aspect, [1.0, 1.0, 1.0, 1.0]);
            ...
            add_quad(&mut verts, -0.30, -0.18, 0.30, -0.10, [0.20, 0.55, 0.30, 1.0]); draw_text_centered(&mut verts, "RESUME (ESC)", 0.0, -0.14, 1.1, aspect, [1.0, 1.0, 1.0, 1.0]);
            add_quad(&mut verts, -0.30, -0.30, 0.30, -0.22, [0.55, 0.20, 0.20, 1.0]); draw_text_centered(&mut verts, "QUIT TO DESKTOP", 0.0, -0.26, 1.1, aspect, [1.0, 1.0, 1.0, 1.0]);
        }
        ActiveMenu::ImportParams => {
            add_quad(&mut verts, -1.0, -1.0, 1.0, 1.0, [0.02, 0.03, 0.05, 0.85]);
            add_quad(&mut verts, -0.426, -0.586, 0.426, 0.626, [0.30, 0.45, 0.65, 1.0]);
            add_quad(&mut verts, -0.42, -0.58, 0.42, 0.62, [0.08, 0.10, 0.14, 0.98]);
            ...
            draw_text(&mut verts, "PLACEMENT ANCHOR:", -0.38, -0.09, 1.0, aspect, [0.8, 0.8, 0.8, 1.0]);
            add_quad(&mut verts, -0.38, -0.19, 0.38, -0.11, [0.18, 0.24, 0.34, 1.0]); draw_text_centered(&mut verts, if glb_settings.place_at_aim { "CROSSHAIR / RAYCAST AIM" } else { "AT PLAYER POSITION" }, 0.0, -0.15, 1.0, aspect, [1.0, 1.0, 1.0, 1.0]);

            add_quad(&mut verts, -0.38, -0.34, 0.38, -0.24, if glb_settings.selected_file.is_some() { [0.20, 0.60, 0.30, 1.0] } else { [0.20, 0.25, 0.25, 0.6] });
            draw_text_centered(&mut verts, "VOXELIZE & INSERT", 0.0, -0.29, 1.15, aspect, [1.0, 1.0, 1.0, 1.0]);
            add_quad(&mut verts, -0.38, -0.48, 0.38, -0.38, [0.45, 0.22, 0.22, 1.0]); draw_text_centered(&mut verts, "CANCEL (ESC)", 0.0, -0.43, 1.1, aspect, [1.0, 1.0, 1.0, 1.0]);
        }
...
impl State {
    async fn new(window: Arc<Window>) -> Self {
        ...
        let adapter = instance.request_adapter(&wgpu::RequestAdapterOptions { 
            power_preference: wgpu::PowerPreference::default(), compatible_surface: Some(&surface), force_fallback_adapter: false, apply_limit_buckets: Default::default(),
        }).await.unwrap();
        ...
        let camera_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: None, contents: bytemuck::cast_slice(&[CameraUniform { view_proj: camera.view_proj(1.0).to_cols_array_2d(), show_borders: 0.0, _pad: [0.0; 3] }]), usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
        });
        ...
        let hotbar_colors = [
            [0.55, 0.55, 0.58], [0.22, 0.75, 0.32], [0.85, 0.20, 0.20], [0.20, 0.48, 0.90], [0.95, 0.78, 0.18],
            [0.15, 0.85, 0.85], [0.82, 0.25, 0.85], [0.95, 0.50, 0.15], [0.95, 0.95, 0.95], [0.20, 0.22, 0.25],
        ];
        ...
        let ui_vertex_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("UI Buffer"), size: (65536 * std::mem::size_of::<UIVertex>()) as wgpu::BufferAddress, usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST, mapped_at_creation: false,
        });
    ...
    pub fn prompt_native_file_dialog(&mut self) {
        if let Some(path) = rfd::FileDialog::new().add_filter("3D Models (*.glb, *.gltf)", &["glb", "gltf"]).pick_file() {
    ...
    pub fn start_nonblocking_voxelization(&mut self) {
        ...
        let pal_arc = Arc::clone(&self.palette);
        std::thread::spawn(move || run_background_voxelization(path, origin, voxel_size, target_height, pal_arc, tx));
    }
    ...
    pub fn focus_on_scene(&mut self) {
        ...
        for (pos, chunk) in &self.chunk_manager.loaded_chunks {
            if chunk.num_indices > 0 {
                ...
                min_bound = min_bound.min(c_min); max_bound = max_bound.max(c_min + Vec3::splat(32.0));
            }
        }
        ...
        for edit in &self.chunk_manager.cube_edits {
            if edit.material != 0 {
                ...
                min_bound = min_bound.min(p); max_bound = max_bound.max(p + Vec3::splat(edit.size));
            }
        }
        ...
        self.camera.position = center - self.camera.forward() * (radius * 2.2).clamp(10.0, 2000.0);
    ...
    pub fn load_game(&mut self, filename: &str) -> std::io::Result<()> {
        ...
        self.play_mode = data.play_mode; self.velocity = Vec3::ZERO; self.hotbar_colors = data.hotbar_colors; *self.palette.write().unwrap() = data.palette;
        self.chunk_manager.world_type = data.world_type; self.chunk_manager.seed = data.seed; self.chunk_manager.cube_edits = data.cube_edits;
        self.chunk_manager.loaded_chunks.clear(); self.chunk_manager.loading_chunks.clear();
    ...
    pub fn scale_voxel_size(&mut self, multiply: bool) { 
        if multiply { self.edit_size *= 2.0; } 
        else { self.edit_size *= 0.5; } 
        self.update_ui(); 
    }
    ...
    fn update_ui(&mut self) {
        ...
        let verts = build_ui_vertices(
            ...
            self.camera.forward(), self.camera.right(), self.camera.up(), self.cursor_free, &self.glb_settings, self.voxelize_progress, &self.voxelize_stage,
        );
    ...
    fn player_collides_at(&self, foot_pos: Vec3, collider: &PlayerCollider) -> bool {
        ...
        let step = MIN_VOXEL_SIZE; 
        
        let mut y = foot_pos.y;
        while y <= foot_pos.y + h {
            let mut x = foot_pos.x - r;
            while x <= foot_pos.x + r {
                let mut z = foot_pos.z - r;
                while z <= foot_pos.z + r {
                    if self.chunk_manager.is_solid(Vec3::new(x, y, z)) { return true; }
                    z += step;
                }
                x += step;
            }
    ...
    fn update(&mut self, dt: f32) {
                ...
                VoxelizeMsg::Done(Ok(chunks)) => {
                    for item in chunks {
                        let chunk = self.chunk_manager.loaded_chunks.entry(item.pos).or_insert_with(|| {
                            let vb = self.device.create_buffer(&wgpu::BufferDescriptor { label: None, size: 1024, usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST, mapped_at_creation: false });
                            let ib = self.device.create_buffer(&wgpu::BufferDescriptor { label: None, size: 1024, usage: wgpu::BufferUsages::INDEX | wgpu::BufferUsages::COPY_DST, mapped_at_creation: false });
        ...
        match self.play_mode {
            ...
            PlayMode::Real => {
                ...
                if self.camera.position.y < void_threshold {
                    self.camera.position.y = surface_y.unwrap_or(12.0) + self.collider.eye_offset;
                    self.velocity = Vec3::ZERO;
                }
                ...
                if self.player_collides_at(foot_pos, &self.collider) {
                    ...
                    for _ in 0..80 {
                        ...
                        let new_foot = self.camera.position - Vec3::new(0.0, self.collider.eye_offset, 0.0);
                        if !self.player_collides_at(new_foot, &self.collider) { freed = true; break; }
                    }
        ...
        if let Some(ref h) = hit { self.last_target = Some([compute_placement_pos(h, s).x, compute_placement_pos(h, s).y, compute_placement_pos(h, s).z]); } 
        else { self.last_target = None; }
        ...
        if !self.cursor_free && (self.input.action_add || self.input.action_remove || self.input.action_pick) {
            if let Some(ref h) = hit {
                if self.input.action_pick {
                    ...
                    if let Some(&color) = pal.get((h.material - 1) as usize) { self.hotbar_colors[self.selected_slot] = color; drop(pal); self.update_ui(); }
                } else if self.input.action_remove { 
                    let p = h.hit_pos - h.normal * (MIN_VOXEL_SIZE * 0.5);
                    self.chunk_manager.modify_cube(Vec3::new((p.x / s).floor() * s, (p.y / s).floor() * s, (p.z / s).floor() * s), 0, s, &self.device, &self.queue); 
                } else if self.input.action_add { 
                    if self.play_mode == PlayMode::Flying || !self.player_collides_at(self.camera.position - Vec3::new(0.0, self.collider.eye_offset, 0.0), &self.collider) {
                        let mat_id = self.get_or_create_material(self.hotbar_colors[self.selected_slot]);
                        self.chunk_manager.modify_cube(compute_placement_pos(h, s), mat_id, s, &self.device, &self.queue); 
                    }
                }
            } else if self.input.action_add {
    ...
    fn render(&mut self) {
...

        {
            let mut render_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("Main Render Pass"), color_attachments: &[Some(wgpu::RenderPassColorAttachment { view: &view, resolve_target: None, ops: wgpu::Operations { load: wgpu::LoadOp::Clear(wgpu::Color { r: 0.12, g: 0.14, b: 0.18, a: 1.0 }), store: wgpu::StoreOp::Store }, depth_slice: None })],
            ...
            for chunk in self.chunk_manager.loaded_chunks.values() {
                if chunk.num_indices > 0 { render_pass.set_vertex_buffer(0, chunk.vertex_buffer.slice(..)); render_pass.set_index_buffer(chunk.index_buffer.slice(..), wgpu::IndexFormat::Uint32); render_pass.draw_indexed(0..chunk.num_indices, 0, 0..1); }
            }
        }
        {
            let mut ui_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("UI Render Pass"), color_attachments: &[Some(wgpu::RenderPassColorAttachment { view: &view, resolve_target: None, ops: wgpu::Operations { load: wgpu::LoadOp::Load, store: wgpu::StoreOp::Store }, depth_slice: None })], depth_stencil_attachment: None, timestamp_writes: None, occlusion_query_set: None, multiview_mask: None,
            });
        ...
        if let Some(state) = self.state.as_mut() {
            ...
            match event {
                ...
                WindowEvent::CursorMoved { position, .. } => {
                    let ndc_x = (position.x as f32 / state.size.width as f32) * 2.0 - 1.0; let ndc_y = 1.0 - (position.y as f32 / state.size.height as f32) * 2.0;
                ...
                WindowEvent::MouseWheel { delta, .. } => {
                    ...
                    if step != 0 {
                        if state.camera.is_ortho && state.cursor_free { 
                            ...
                            state.update_ui(); 
                        } else { state.selected_slot = (state.selected_slot as i32 + step).rem_euclid(10) as usize; state.update_ui(); }
                    }
                }
                WindowEvent::KeyboardInput { event: key_event, .. } => {
                    ...
                    if key_event.physical_key == PhysicalKey::Code(KeyCode::KeyE) && is_pressed {
                        if state.active_menu == ActiveMenu::Edit { state.set_menu(ActiveMenu::None); } else if state.active_menu == ActiveMenu::None { state.set_menu(ActiveMenu::Edit); }
                        return;
                    }

                    if key_event.physical_key == PhysicalKey::Code(KeyCode::Escape) && is_pressed {
                        match state.active_menu { ActiveMenu::None | ActiveMenu::ImportParams => state.set_menu(ActiveMenu::Pause), ActiveMenu::Voxelizing => {}, _ => state.set_menu(ActiveMenu::None) }
                        return;
                    }
                    ...
                    if state.active_menu == ActiveMenu::None {
                        match key_event.physical_key {
                            PhysicalKey::Code(KeyCode::KeyW) => state.input.forward = is_pressed, PhysicalKey::Code(KeyCode::KeyS) => state.input.backward = is_pressed,
                            PhysicalKey::Code(KeyCode::KeyA) => state.input.left = is_pressed, PhysicalKey::Code(KeyCode::KeyD) => state.input.right = is_pressed,
                            PhysicalKey::Code(KeyCode::Space) => state.input.up = is_pressed, PhysicalKey::Code(KeyCode::ShiftLeft) => state.input.down = is_pressed,
                            _ => {}
                        }
                    }
                }
                    ...
                    if button == MouseButton::Left {
                        if element_state == ElementState::Pressed {
                            ...
                            if !state.gimbal_drag_moved {
                                let right = state.camera.right(); let up = state.camera.up();
                                ...
                                for ax in sorted_axes {
                                    if ((mx - (GIZMO_CENTER_X + (ax.dir.dot(right) * GIZMO_RADIUS) / aspect)) * aspect).powi(2) + (my - (GIZMO_CENTER_Y + (ax.dir.dot(up) * GIZMO_RADIUS))).powi(2) <= 0.001225 {
                                        state.camera.yaw = ax.yaw; state.camera.pitch = ax.pitch; state.camera.is_ortho = true; state.update_camera_buffer(); state.update_ui(); return;
                                    }
                                }
                            }
                            return;
                    ...
                    match state.active_menu {
                        ...
                        ActiveMenu::Pause => {
                            if button == MouseButton::Left && element_state == ElementState::Pressed {
                                ...
                                if mx >= -0.30 && mx <= 0.30 && my >= -0.18 && my <= -0.10 { state.set_menu(ActiveMenu::None); return; }
                                if mx >= -0.30 && mx <= 0.30 && my >= -0.30 && my <= -0.22 { event_loop.exit(); return; }
                            }
                        }
                        ActiveMenu::ImportParams => {
                            if button == MouseButton::Left && element_state == ElementState::Pressed {
                                ...
                                if my >= -0.01 && my <= 0.07 { 
                                    if mx >= wx0 + 0.04 && mx <= wx0 + 0.14 { state.glb_settings.voxel_size *= 0.5; state.update_ui(); return; } 
                                    if mx >= wx1 - 0.14 && mx <= wx1 - 0.04 { state.glb_settings.voxel_size *= 2.0; state.update_ui(); return; } 
                                }
                                if mx >= wx0 + 0.04 && mx <= wx1 - 0.04 && my >= -0.19 && my <= -0.11 { state.glb_settings.place_at_aim = !state.glb_settings.place_at_aim; state.update_ui(); return; }
                                if mx >= wx0 + 0.04 && mx <= wx1 - 0.04 && my >= -0.34 && my <= -0.24 { state.start_nonblocking_voxelization(); return; }
                                if mx >= wx0 + 0.04 && mx <= wx1 - 0.04 && my >= -0.48 && my <= -0.38 { state.set_menu(ActiveMenu::Pause); return; }
                            }
                        }
                        ActiveMenu::Voxelizing => {}
                        ActiveMenu::None => {
