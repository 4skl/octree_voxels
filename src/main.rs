...
impl State {
    async fn new(window: Arc<Window>) -> Self {
        ...
        let mut app_state = Self {
            window, surface, device, queue, config, size, render_pipeline, ui_pipeline, ui_vertex_buffer, ui_vertices_count: 0, camera_buffer, camera_bind_group, depth_texture_view, camera,
    ...
    pub fn prompt_native_file_dialog(&mut self) { if let Some(path) = rfd::FileDialog::new().add_filter("3D Models", &["glb", "gltf"]).pick_file() { self.glb_settings.selected_file = Some(path); self.set_menu(ActiveMenu::ImportParams); } }
    pub fn start_nonblocking_voxelization(&mut self) {
        ...
        let (tx, rx) = mpsc::channel(); self.voxelize_rx = Some(rx); self.voxelize_progress = 0.0; self.voxelize_stage = "INITIALIZING...".into(); self.set_menu(ActiveMenu::Voxelizing);
    ...
    fn player_collides_at(&self, foot_pos: Vec3, collider: &PlayerCollider) -> bool {
        ...
        let min = foot_pos - Vec3::new(r, 0.0, r); let max = foot_pos + Vec3::new(r, h, r);
        ...
        if let Some(state) = self.state.as_mut() {
            ...
            match event {
                ...
                WindowEvent::KeyboardInput { event: key_event, .. } => {
                    ...
                    if key_event.physical_key == PhysicalKey::Code(KeyCode::Escape) && is_pressed { match state.active_menu { ActiveMenu::None | ActiveMenu::ImportParams | ActiveMenu::Controls => state.set_menu(ActiveMenu::Pause), ActiveMenu::Voxelizing => {}, _ => state.set_menu(ActiveMenu::None) } return; }
                    if is_pressed {
                        match key_event.physical_key {
