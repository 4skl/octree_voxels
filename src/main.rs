...
const GRID_RES: u32 = 1 << MAX_DEPTH;
const MIN_VOXEL_SIZE: f32 = CHUNK_SIZE / (GRID_RES as f32);
...
fn sample_triangle(pts: [Vec3; 3], color: [f32; 3], voxel_size: f32, out: &mut Vec<(u64, [f32; 3])>) {
    let min_p = pts[0].min(pts[1]).min(pts[2]); let max_p = pts[0].max(pts[1]).max(pts[2]);
    let min_gx = (min_p.x / voxel_size).floor() as i32; let max_gx = (max_p.x / voxel_size).floor() as i32;
    let min_gy = (min_p.y / voxel_size).floor() as i32; let max_gy = (max_p.y / voxel_size).floor() as i32;
    let min_gz = (min_p.z / voxel_size).floor() as i32; let max_gz = (max_p.z / voxel_size).floor() as i32;
    let ext = voxel_size * 0.5; let box_half = Vec3::splat(ext);
    for gx in min_gx..=max_gx {
        for gy in min_gy..=max_gy {
            for gz in min_gz..=max_gz {
                ...
                let test_axis = |axis: Vec3| -> bool { let p0 = v0.dot(axis); let p1 = v1.dot(axis); let p2 = v2.dot(axis); let rad = box_half.x * axis.x.abs() + box_half.y * axis.y.abs() + box_half.z * axis.z.abs(); p0.min(p1).min(p2) > rad || p0.max(p1).max(p2) < -rad };
...
fn run_background_voxelization(
    ...
    palette_size: usize, palette_arc: Arc<RwLock<Palette>>, tx: mpsc::Sender<VoxelizeMsg>
) {
    let _ = tx.send(VoxelizeMsg::Progress { percent: 0.05, stage: "READING GLB CONTAINER...".into() });
    let (document, buffers, _) = match gltf::import(&path) { Ok(res) => res, Err(e) => { let _ = tx.send(VoxelizeMsg::Done(Err(format!("Import error: {}", e)))); return; } };
    let mut min_bound = Vec3::splat(f32::MAX); let mut max_bound = Vec3::splat(f32::MIN);
    let mut all_triangles = Vec::new();
    let mut root_nodes: Vec<gltf::Node> = Vec::new();
    if let Some(scene) = document.default_scene() { root_nodes.extend(scene.nodes()); } else if let Some(scene) = document.scenes().next() { root_nodes.extend(scene.nodes()); } else {
    ...
    while let Some((node, parent_mat)) = stack.pop() {
        let local_mat = Mat4::from_cols_array_2d(&node.transform().matrix()); let world_mat = parent_mat * local_mat;
        if let Some(mesh) = node.mesh() {
            for primitive in mesh.primitives() {
                let reader = primitive.reader(|b| Some(&buffers[b.index()]));
                let positions: Vec<Vec3> = match reader.read_positions() { Some(iter) => iter.map(|p| world_mat.transform_point3(Vec3::from(p))).collect(), None => continue };
                ...
                let pbr = primitive.material().pbr_metallic_roughness(); let base_factor = pbr.base_color_factor();
    ...
    if all_triangles.is_empty() { let _ = tx.send(VoxelizeMsg::Done(Err("No valid mesh geometry found in GLB".into()))); return; }
    let extent = max_bound - min_bound; let max_dim = extent.x.max(extent.y).max(extent.z).max(0.001);
    let scale = target_height / max_dim; let center_x = (min_bound.x + max_bound.x) * 0.5; let center_z = (min_bound.z + max_bound.z) * 0.5; let min_y = min_bound.y;
    ...
    let total_triangles = normalized_triangles.len(); let processed = Arc::new(AtomicUsize::new(0)); let processed_clone = Arc::clone(&processed); let tx_progress = tx.clone();
    let reporter = std::thread::spawn(move || {
        loop {
            ...
            if tx_progress.send(VoxelizeMsg::Progress { percent: pct.min(0.70), stage: format!("VOXELIZING: {}/{} TRIS ({:.0}%)", count, total_triangles, (count as f32 / total_triangles.max(1) as f32) * 100.0) }).is_err() || count >= total_triangles { break; }
        }
    });
    let sampled_batches: Vec<Vec<(u64, [f32; 3])>> = normalized_triangles.par_chunks(2048).map(|chunk| {
        ...
        local_out.sort_unstable_by_key(|&(k, _)| k); local_out.dedup_by(|a, b| a.0 == b.0);
        processed.fetch_add(chunk.len(), Ordering::Relaxed); local_out
    }).collect();
    let _ = reporter.join();
    let _ = tx.send(VoxelizeMsg::Progress { percent: 0.72, stage: "DEDUPLICATING SURFACE VOXELS...".into() });
    let mut all_samples: Vec<(u64, [f32; 3])> = sampled_batches.into_par_iter().flatten().collect();
    all_samples.par_sort_unstable_by_key(|&(k, _)| k); all_samples.dedup_by(|a, b| a.0 == b.0);
    let _ = tx.send(VoxelizeMsg::Progress { percent: 0.78, stage: format!("RESOLVING PALETTE FOR {} VOXELS...", all_samples.len()) });
    
    let mut pal = palette_arc.write().unwrap();
    let mut color_cache: HashMap<[u8; 3], u16> = HashMap::new();
    for (i, &c) in pal.iter().enumerate() { color_cache.insert([(c[0] * 255.0).round() as u8, (c[1] * 255.0).round() as u8, (c[2] * 255.0).round() as u8], (i + 1) as u16); }
    ...
    for (k, color) in all_samples {
        ...
        let mat_id = if let Some(&id) = color_cache.get(&key) { id } else {
            if pal.len() < palette_size {
                pal.push(color); let new_id = pal.len() as u16; color_cache.insert(key, new_id); new_id
            } else {
                let mut best_id = 1; let mut best_dist = f32::MAX;
                for (i, c) in pal.iter().enumerate() {
                    let dist = (c[0]-color[0]).powi(2) + (c[1]-color[1]).powi(2) + (c[2]-color[2]).powi(2);
                    if dist < best_dist { best_dist = dist; best_id = (i + 1) as u16; }
                }
                color_cache.insert(key, best_id); best_id
            }
        };

        let (gx, gy, gz) = unpack_coords(k);
        let wx = gx as f32 * voxel_size; let wy = gy as f32 * voxel_size; let wz = gz as f32 * voxel_size;
        let cx = (wx / 32.0).floor() as i32; let cy = (wy / 32.0).floor() as i32; let cz = (wz / 32.0).floor() as i32;
        let lx = wx - (cx * 32) as f32; let ly = wy - (cy * 32) as f32; let lz = wz - (cz * 32) as f32;
    ...
    let pal_snapshot = pal.clone(); drop(pal);
    let _ = tx.send(VoxelizeMsg::Progress { percent: 0.85, stage: format!("PARALLEL MESHING {} CHUNKS...", chunks_voxels.len()) });

    let completed_chunks: Vec<ImportedChunkData> = chunks_voxels.into_par_iter().map(|(chunk_pos, voxels)| {
        let mut octree = Octree::new();
        for (x, y, z, mat) in voxels { octree.insert_cube(x, y, z, depth, mat, false); }
        octree.collapse(octree.root_index as usize); let mesh = generate_mesh(&octree, chunk_pos, &pal_snapshot);
        ImportedChunkData { pos: chunk_pos, octree, mesh }
    }).collect();

    let _ = tx.send(VoxelizeMsg::Progress { percent: 0.99, stage: "DISPATCHING TO GPU...".into() });
    let _ = tx.send(VoxelizeMsg::Done(Ok(completed_chunks)));
}

pub struct MeshPayload { vertices: Vec<Vertex>, indices: Vec<u32> }

fn generate_octree(chunk_pos: ChunkPos, world_type: WorldType, seed: u32, edits: &[CubeEdit], lod_level: u8) -> Octree {
    let mut octree = Octree::new();
...
fn emit_quad(vertices: &mut Vec<Vertex>, indices: &mut Vec<u32>, corners: [[f32; 3]; 4], normal: [f32; 3], color: [f32; 3]) {
    let start = vertices.len() as u32;
    for (i, &pos) in corners.iter().enumerate() { vertices.push(Vertex { position: pos, normal, color, uv: QUAD_UVS[i] }); }
    indices.extend_from_slice(&[start, start + 1, start + 2, start, start + 2, start + 3]);
}

fn is_face_occluded(octree: &Octree, gx: u32, gy: u32, gz: u32, size: u32, axis: usize, positive: bool) -> bool {
...
fn mesh_octree_node(octree: &Octree, node_idx: usize, gx: u32, gy: u32, gz: u32, size: u32, chunk_pos: ChunkPos, palette: &[[f32; 3]], vertices: &mut Vec<Vertex>, indices: &mut Vec<u32>) {
...
pub struct RenderChunk {
    octree: Octree, vertex_buffer: wgpu::Buffer, index_buffer: wgpu::Buffer, num_indices: u32, vertex_capacity: usize, index_capacity: usize,
}
...
fn ray_aabb_intersect(ray_origin: Vec3, ray_dir_inv: Vec3, min: Vec3, max: Vec3) -> Option<(f32, Vec3)> {
...
fn check_octree_aabb(octree: &Octree, node_idx: usize, node_min: Vec3, node_size: f32, target_min: Vec3, target_max: Vec3) -> bool {
    let node = &octree.nodes[node_idx];
...
pub struct ChunkManager {
    loaded_chunks: HashMap<ChunkPos, RenderChunk>, loading_chunks: HashSet<ChunkPos>,
    tx: mpsc::Sender<(ChunkPos, Octree, MeshPayload)>, rx: mpsc::Receiver<(ChunkPos, Octree, MeshPayload)>,
    render_distance: i32, palette: Arc<RwLock<Palette>>, pub world_type: WorldType, pub seed: u32, pub cube_edits: Vec<CubeEdit>,
}
...
impl ChunkManager {
    ...
    fn update(&mut self, player_pos: Vec3, device: &wgpu::Device) {
        ...
        for x in -self.render_distance..=self.render_distance { for z in -self.render_distance..=self.render_distance {
            let dist_sq = x*x + z*z; let lod_level = if dist_sq < 9 { 0 } else if dist_sq < 25 { 1 } else { 2 };
    ...
    fn is_solid(&self, pos: Vec3) -> bool { self.get_voxel_info_at(pos).0 != 0 }
...
impl Vertex {
    ...
    fn desc() -> wgpu::VertexBufferLayout<'static> { wgpu::VertexBufferLayout { array_stride: std::mem::size_of::<Vertex>() as wgpu::BufferAddress, step_mode: wgpu::VertexStepMode::Vertex, attributes: &Self::ATTRIBS } }
}
...
struct CameraUniform { view_proj: [[f32; 4]; 4], show_borders: f32, _pad: [f32; 3] }

struct Camera { position: Vec3, yaw: f32, pitch: f32, is_ortho: bool, ortho_size: f32 }
impl Camera {
    ...
    fn forward(&self) -> Vec3 { let (sin_p, cos_p) = self.pitch.sin_cos(); let (sin_y, cos_y) = self.yaw.sin_cos(); Vec3::new(cos_y * cos_p, sin_p, sin_y * cos_p).normalize() }
    fn right(&self) -> Vec3 { let (sin_y, cos_y) = self.yaw.sin_cos(); Vec3::new(-sin_y, 0.0, cos_y).normalize() }
    fn up(&self) -> Vec3 { self.right().cross(self.forward()).normalize() }
}
...
fn compute_placement_pos(hit: &RaycastHit, edit_size: f32) -> Vec3 {
    let s = edit_size; let mut pos = Vec3::ZERO;
    if hit.normal.x > 0.5 { let face_x = hit.voxel_min.x + hit.voxel_size; pos.x = (face_x / s).ceil() * s; pos.y = (hit.hit_pos.y / s).floor() * s; pos.z = (hit.hit_pos.z / s).floor() * s; }
    else if hit.normal.x < -0.5 { let face_x = hit.voxel_min.x; pos.x = ((face_x - s) / s).floor() * s; pos.y = (hit.hit_pos.y / s).floor() * s; pos.z = (hit.hit_pos.z / s).floor() * s; }
    else if hit.normal.y > 0.5 { let face_y = hit.voxel_min.y + hit.voxel_size; pos.y = (face_y / s).ceil() * s; pos.x = (hit.hit_pos.x / s).floor() * s; pos.z = (hit.hit_pos.z / s).floor() * s; }
    else if hit.normal.y < -0.5 { let face_y = hit.voxel_min.y; pos.y = ((face_y - s) / s).floor() * s; pos.x = (hit.hit_pos.x / s).floor() * s; pos.z = (hit.hit_pos.z / s).floor() * s; }
    else if hit.normal.z > 0.5 { let face_z = hit.voxel_min.z + hit.voxel_size; pos.z = (face_z / s).ceil() * s; pos.x = (hit.hit_pos.x / s).floor() * s; pos.y = (hit.hit_pos.y / s).floor() * s; }
    else if hit.normal.z < -0.5 { let face_z = hit.voxel_min.z; pos.z = ((face_z - s) / s).floor() * s; pos.x = (hit.hit_pos.x / s).floor() * s; pos.y = (hit.hit_pos.y / s).floor() * s; }
    else { pos = (hit.hit_pos / s).floor() * s; }
    pos
}
...
fn build_ui_vertices(
    ...
    cursor_free: bool, glb_settings: &GlbImportSettings, progress_val: f32, progress_stage: &str, view_proj: Mat4
) -> Vec<UIVertex> {
    ...
    match active_menu {
        ActiveMenu::None => {
            ...
            draw_text(&mut verts, &format!("MODE: {} | PROJ: {} | WORLD: {} | {}", if play_mode == PlayMode::Flying { "FLY" } else { "REAL" }, if is_ortho { "ORTHO" } else { "PERSP" }, world_type.name(), size_str), -0.96, 0.92, 1.25, aspect, [1.0, 1.0, 1.0, 0.95]);
            if cursor_free { draw_text(&mut verts, "CURSOR FREE  |  DRAG & DROP .GLB OR OPEN ESC MENU", -0.96, 0.86, 1.0, aspect, [0.95, 0.45, 0.2, 0.95]); } else if let Some(tpos) = target_pos { draw_text(&mut verts, &format!("AIM: [{:.3}, {:.3}, {:.3}] (SNAP)", tpos[0], tpos[1], tpos[2]), -0.96, 0.86, 1.0, aspect, [0.3, 0.9, 0.9, 0.9]); } else { draw_text(&mut verts, "AIM: [UNBOUNDED VOID - PLACES IN FRONT]", -0.96, 0.86, 1.0, aspect, [0.6, 0.7, 0.8, 0.8]); }
            draw_text(&mut verts, "[E] PALETTE  [TAB] FREE  [P] ORTHO  [.] FOCUS  [ESC] MENU  [F1] HIDE UI", -0.96, 0.80, 0.95, aspect, [0.9, 0.85, 0.4, 0.85]);
        }
        ActiveMenu::Edit => {
        ...
        ActiveMenu::Pause => {
            ...
            add_quad(&mut verts, -0.426, -0.726, 0.426, 0.726, [0.45, 0.45, 0.50, 1.0]);
            add_quad(&mut verts, -0.42, -0.72, 0.42, 0.72, [0.12, 0.13, 0.17, 0.98]);
        ...
        ActiveMenu::ImportParams => {
            ...
            add_quad(&mut verts, -0.38, -0.54, 0.38, -0.44, if glb_settings.selected_file.is_some() { [0.20, 0.60, 0.30, 1.0] } else { [0.20, 0.25, 0.25, 0.6] });
            draw_text_centered(&mut verts, "VOXELIZE & INSERT", 0.0, -0.49, 1.15, aspect, [1.0, 1.0, 1.0, 1.0]);
        ...
        ActiveMenu::Voxelizing => {
            ...
            draw_text_centered(&mut verts, &format!("{:.0}%", (progress_val * 100.0).clamp(0.0, 100.0)), 0.0, -0.10, 1.1, aspect, [1.0, 1.0, 1.0, 1.0]); draw_text_centered(&mut verts, "MULTI-THREAD WORKER ACTIVE", 0.0, -0.18, 0.85, aspect, [0.5, 0.8, 0.6, 0.9]);
        }
    }
    verts
}
...
struct State {
    ...
    camera_buffer: wgpu::Buffer, camera_bind_group: wgpu::BindGroup, depth_texture_view: wgpu::TextureView, window: Arc<Window>, camera: Camera, input: InputState, chunk_manager: ChunkManager,
...
impl State {
    async fn new(window: Arc<Window>) -> Self {
        ...
        let camera_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor { label: None, contents: bytemuck::cast_slice(&[CameraUniform { view_proj: camera.view_proj(1.0).to_cols_array_2d(), show_borders: 0.0, _pad: [0.0; 3] }]), usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST });
        let camera_bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor { entries: &[wgpu::BindGroupLayoutEntry { binding: 0, visibility: wgpu::ShaderStages::VERTEX | wgpu::ShaderStages::FRAGMENT, count: None, ty: wgpu::BindingType::Buffer { ty: wgpu::BufferBindingType::Uniform, has_dynamic_offset: false, min_binding_size: None } }], label: None });
        ...
        let depth_texture_view = device.create_texture(&wgpu::TextureDescriptor { size: wgpu::Extent3d { width: config.width, height: config.height, depth_or_array_layers: 1 }, mip_level_count: 1, sample_count: 1, dimension: wgpu::TextureDimension::D2, format: DEPTH_FORMAT, usage: wgpu::TextureUsages::RENDER_ATTACHMENT, label: None, view_formats: &[] }).create_view(&wgpu::TextureViewDescriptor::default());
        ...
        let ui_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            ...
            primitive: wgpu::PrimitiveState { topology: wgpu::PrimitiveTopology::TriangleList, cull_mode: None, ..Default::default() }, depth_stencil: None, multisample: wgpu::MultisampleState::default(), multiview_mask: None, cache: None,
        });

        let hotbar_colors = [[0.55, 0.55, 0.58], [0.22, 0.75, 0.32], [0.85, 0.20, 0.20], [0.20, 0.48, 0.90], [0.95, 0.78, 0.18], [0.15, 0.85, 0.85], [0.82, 0.25, 0.85], [0.95, 0.50, 0.15], [0.95, 0.95, 0.95], [0.20, 0.22, 0.25]];
        let palette = Arc::new(RwLock::new(hotbar_colors.to_vec())); let chunk_manager = ChunkManager::new(Arc::clone(&palette)); let glb_settings = GlbImportSettings::default();

        let initial_ui = build_ui_vertices(0, ActiveMenu::None, &hotbar_colors, PlayMode::Flying, camera.is_ortho, chunk_manager.world_type, 1.0, None, 1.0, camera.forward(), camera.right(), camera.up(), false, &glb_settings, 0.0, "", camera.view_proj(1.0));
        let ui_vertices_count = initial_ui.len() as u32;
        let ui_vertex_buffer = device.create_buffer(&wgpu::BufferDescriptor { label: Some("UI Buffer"), size: (65536 * std::mem::size_of::<UIVertex>()) as wgpu::BufferAddress, usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST, mapped_at_creation: false });
        queue.write_buffer(&ui_vertex_buffer, 0, bytemuck::cast_slice(&initial_ui));

        Self {
            window, surface, device, queue, config, size, render_pipeline, ui_pipeline, ui_vertex_buffer, ui_vertices_count, camera_buffer, camera_bind_group, depth_texture_view, camera,
            input: InputState::default(), chunk_manager, play_mode: PlayMode::Flying, velocity: Vec3::ZERO, selected_slot: 0, hotbar_colors, palette,
            active_menu: ActiveMenu::None, cursor_pos: [0.0, 0.0], active_slider: None, edit_size: 1.0, last_target: None, cursor_free: false, gimbal_dragging: false, gimbal_drag_moved: false, prev_cursor_pos: [0.0, 0.0],
            glb_settings, voxelize_rx: None, voxelize_progress: 0.0, voxelize_stage: String::new(), collider: PlayerCollider::default(), hide_ui: false, bg_color: [0.12, 0.14, 0.18],
        }
    }
    pub fn prompt_native_file_dialog(&mut self) { if let Some(path) = rfd::FileDialog::new().add_filter("3D Models (*.glb, *.gltf)", &["glb", "gltf"]).pick_file() { self.glb_settings.selected_file = Some(path); self.set_menu(ActiveMenu::ImportParams); } }
    pub fn start_nonblocking_voxelization(&mut self) {
        let path = match self.glb_settings.selected_file.clone() { Some(p) => p, None => return };
        let origin = if self.glb_settings.place_at_aim { self.last_target.map(Vec3::from).unwrap_or_else(|| self.camera.position + self.camera.forward() * 12.0) } else { self.camera.position - Vec3::new(0.0, self.collider.eye_offset, 0.0) };
        let voxel_size = self.glb_settings.voxel_size.max(MIN_VOXEL_SIZE); let target_height = self.glb_settings.target_height; let palette_size = self.glb_settings.palette_size;
        let (tx, rx) = mpsc::channel(); self.voxelize_rx = Some(rx); self.voxelize_progress = 0.0; self.voxelize_stage = "INITIALIZING THREAD POOL...".into(); self.set_menu(ActiveMenu::Voxelizing);
        let pal_arc = Arc::clone(&self.palette);
        std::thread::spawn(move || run_background_voxelization(path, origin, voxel_size, target_height, palette_size, pal_arc, tx));
    }
    pub fn focus_on_scene(&mut self) {
        let mut min_bound = Vec3::splat(f32::MAX); let mut max_bound = Vec3::splat(f32::MIN); let mut has_blocks = false;
        for (pos, chunk) in &self.chunk_manager.loaded_chunks { if chunk.num_indices > 0 { has_blocks = true; let c_min = Vec3::new((pos.0 * 32) as f32, (pos.1 * 32) as f32, (pos.2 * 32) as f32); min_bound = min_bound.min(c_min); max_bound = max_bound.max(c_min + Vec3::splat(32.0)); } }
        for edit in &self.chunk_manager.cube_edits { if edit.material != 0 { has_blocks = true; let p = Vec3::from_array(edit.pos); min_bound = min_bound.min(p); max_bound = max_bound.max(p + Vec3::splat(edit.size)); } }
        let (center, radius) = if has_blocks { ((min_bound + max_bound) * 0.5, ((max_bound - min_bound).length() * 0.5).max(4.0)) } else { (Vec3::new(16.0, 0.0, 16.0), 16.0) };
        self.camera.position = center - self.camera.forward() * (radius * 2.2).clamp(10.0, 2000.0); self.camera.ortho_size = (radius * 2.5).clamp(8.0, 2000.0); self.velocity = Vec3::ZERO;
        self.update_camera_buffer(); self.update_ui();
    }
    pub fn toggle_projection(&mut self) { self.camera.is_ortho = !self.camera.is_ortho; self.update_camera_buffer(); self.update_ui(); }
    pub fn update_camera_buffer(&self) {
        ...
        self.queue.write_buffer(&self.camera_buffer, 0, bytemuck::cast_slice(&[CameraUniform { view_proj: self.camera.view_proj(aspect).to_cols_array_2d(), show_borders: if self.cursor_free || self.active_menu == ActiveMenu::Edit { 1.0 } else { 0.0 }, _pad: [0.0; 3] }]));
    }
    pub fn set_cursor_mode(&mut self, free: bool) {
        self.cursor_free = free;
        if free || self.active_menu != ActiveMenu::None { let _ = self.window.set_cursor_grab(winit::window::CursorGrabMode::None); self.window.set_cursor_visible(true); self.input = InputState::default(); } 
        else { let _ = self.window.set_cursor_grab(winit::window::CursorGrabMode::Locked).or_else(|_| self.window.set_cursor_grab(winit::window::CursorGrabMode::Confined)); self.window.set_cursor_visible(false); }
        self.update_camera_buffer(); self.update_ui();
    }
    pub fn set_menu(&mut self, menu: ActiveMenu) { self.active_menu = menu; self.active_slider = None; self.gimbal_dragging = false; self.set_cursor_mode(self.active_menu != ActiveMenu::None); }
    pub fn save_game(&self, filename: &str) -> std::io::Result<()> {
        std::fs::write(filename, serde_json::to_string_pretty(&SaveData {
            player_pos: self.camera.position.to_array(), camera_yaw: self.camera.yaw, camera_pitch: self.camera.pitch, is_ortho: self.camera.is_ortho, ortho_size: self.camera.ortho_size,
            play_mode: self.play_mode, world_type: self.chunk_manager.world_type, seed: self.chunk_manager.seed, hotbar_colors: self.hotbar_colors, palette: self.palette.read().unwrap().clone(), cube_edits: self.chunk_manager.cube_edits.clone(),
        })?)?; Ok(())
    }
    pub fn load_game(&mut self, filename: &str) -> std::io::Result<()> {
        let data: SaveData = serde_json::from_str(&std::fs::read_to_string(filename)?)?;
        self.camera.position = Vec3::from_array(data.player_pos); self.camera.yaw = data.camera_yaw; self.camera.pitch = data.camera_pitch; self.camera.is_ortho = data.is_ortho; self.camera.ortho_size = if data.ortho_size > 0.1 { data.ortho_size } else { 36.0 };
        self.play_mode = data.play_mode; self.velocity = Vec3::ZERO; self.hotbar_colors = data.hotbar_colors; *self.palette.write().unwrap() = data.palette; self.chunk_manager.world_type = data.world_type; self.chunk_manager.seed = data.seed; self.chunk_manager.cube_edits = data.cube_edits;
        self.chunk_manager.loaded_chunks.clear(); self.chunk_manager.loading_chunks.clear(); self.update_camera_buffer(); self.update_ui(); Ok(())
    }
    pub fn clear_all_blocks(&mut self) { self.chunk_manager.cube_edits.clear(); self.chunk_manager.loaded_chunks.clear(); self.chunk_manager.loading_chunks.clear(); self.update_ui(); }
    pub fn cycle_world_generator(&mut self) { self.chunk_manager.world_type = self.chunk_manager.world_type.next(); self.clear_all_blocks(); }
    pub fn scale_voxel_size(&mut self, multiply: bool) { if multiply { self.edit_size *= 2.0; } else { self.edit_size *= 0.5; } self.update_ui(); }
    fn toggle_play_mode(&mut self) { self.play_mode = if self.play_mode == PlayMode::Real { PlayMode::Flying } else { PlayMode::Real }; self.velocity = Vec3::ZERO; self.update_ui(); }
    fn get_or_create_material(&self, color: [f32; 3]) -> u16 {
        let mut pal = self.palette.write().unwrap();
        for (i, &c) in pal.iter().enumerate() { if (c[0] - color[0]).abs() < 0.005 && (c[1] - color[1]).abs() < 0.005 && (c[2] - color[2]).abs() < 0.005 { return (i + 1) as u16; } }
        pal.push(color); pal.len() as u16
    }
    fn update_ui(&mut self) {
        let aspect = if self.config.height > 0 { self.config.width as f32 / self.config.height as f32 } else { 1.0 };
        let verts = build_ui_vertices(
            self.selected_slot, self.active_menu, &self.hotbar_colors, self.play_mode, self.camera.is_ortho, self.chunk_manager.world_type, self.edit_size, self.last_target, aspect,
            self.camera.forward(), self.camera.right(), self.camera.up(), self.cursor_free, &self.glb_settings, self.voxelize_progress, &self.voxelize_stage, self.camera.view_proj(aspect)
        );
        self.ui_vertices_count = verts.len() as u32; self.queue.write_buffer(&self.ui_vertex_buffer, 0, bytemuck::cast_slice(&verts));
    }
    fn player_collides_at(&self, foot_pos: Vec3, collider: &PlayerCollider) -> bool {
        ...
        self.chunk_manager.check_aabb_collision(min, max)
    }
    fn resize(&mut self, new_size: winit::dpi::PhysicalSize<u32>) {
        if new_size.width > 0 && new_size.height > 0 {
            self.size = new_size; self.config.width = new_size.width; self.config.height = new_size.height;
            self.surface.configure(&self.device, &self.config);
            self.depth_texture_view = self.device.create_texture(&wgpu::TextureDescriptor { size: wgpu::Extent3d { width: self.config.width, height: self.config.height, depth_or_array_layers: 1 }, mip_level_count: 1, sample_count: 1, dimension: wgpu::TextureDimension::D2, format: DEPTH_FORMAT, usage: wgpu::TextureUsages::RENDER_ATTACHMENT, label: None, view_formats: &[] }).create_view(&wgpu::TextureViewDescriptor::default());
    ...
    fn update(&mut self, dt: f32) {
        ...
        for msg in messages {
            match msg {
                VoxelizeMsg::Progress { percent, stage } => { self.voxelize_progress = percent; self.voxelize_stage = stage; self.update_ui(); }
                VoxelizeMsg::Done(Ok(chunks)) => {
                    for item in chunks {
                        let chunk = self.chunk_manager.loaded_chunks.entry(item.pos).or_insert_with(|| { let vb = self.device.create_buffer(&wgpu::BufferDescriptor { label: None, size: 1024, usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST, mapped_at_creation: false }); let ib = self.device.create_buffer(&wgpu::BufferDescriptor { label: None, size: 1024, usage: wgpu::BufferUsages::INDEX | wgpu::BufferUsages::COPY_DST, mapped_at_creation: false }); RenderChunk { octree: item.octree.clone(), vertex_buffer: vb, index_buffer: ib, num_indices: 0, vertex_capacity: 1024, index_capacity: 1024 } });
                        chunk.octree = item.octree; update_chunk_buffers(chunk, &item.mesh, &self.device, &self.queue);
                    }
                    self.voxelize_rx = None; self.set_menu(ActiveMenu::None); self.focus_on_scene(); return;
                }
                VoxelizeMsg::Done(Err(err)) => { eprintln!("GLB voxelization failed: {}", err); self.voxelize_rx = None; self.set_menu(ActiveMenu::ImportParams); return; }
            }
        }
        if self.active_menu != ActiveMenu::None { return; }

        let (sin_y, cos_y) = self.camera.yaw.sin_cos();
        let forward = Vec3::new(cos_y, 0.0, sin_y).normalize(); let right = Vec3::new(-sin_y, 0.0, cos_y).normalize();
        ...
        match self.play_mode {
            PlayMode::Flying => {
                let speed = 28.0; if self.input.up { movement.y += 1.0; } if self.input.down { movement.y -= 1.0; }
            ...
            PlayMode::Real => {
                ...
                if self.camera.position.y < void_threshold { self.camera.position.y = surface_y.unwrap_or(12.0) + self.collider.eye_offset; self.velocity = Vec3::ZERO; }
                if self.player_collides_at(foot_pos, &self.collider) {
                    let mut freed = false;
                    for _ in 0..80 { self.camera.position.y += 0.25; let new_foot = self.camera.position - Vec3::new(0.0, self.collider.eye_offset, 0.0); if !self.player_collides_at(new_foot, &self.collider) { freed = true; break; } }
                    if !freed { if let Some(top_y) = surface_y { self.camera.position.y = top_y + self.collider.eye_offset; } }
                    self.velocity.y = 0.0;
                }
                let walk_speed = 6.5; if movement.length_squared() > 0.0 { movement = movement.normalize(); }
                let dx = movement.x * walk_speed * dt; if !self.player_collides_at(foot_pos + Vec3::new(dx, 0.0, 0.0), &self.collider) { self.camera.position.x += dx; }
                let dz = movement.z * walk_speed * dt; if !self.player_collides_at(foot_pos + Vec3::new(0.0, 0.0, dz), &self.collider) { self.camera.position.z += dz; }
                ...
                let total_dy = self.velocity.y * dt; let step_count = ((total_dy.abs() / 0.08).ceil() as i32).max(1); let step_dy = total_dy / step_count as f32;
                for _ in 0..step_count { if self.player_collides_at(foot_pos + Vec3::new(0.0, step_dy, 0.0), &self.collider) { self.velocity.y = 0.0; break; } else { self.camera.position.y += step_dy; } }
            }
        }

        let dir = self.camera.forward(); let hit = self.chunk_manager.raycast(self.camera.position, dir, 300.0); let s = self.edit_size;
        if let Some(ref h) = hit { self.last_target = Some([compute_placement_pos(h, s).x, compute_placement_pos(h, s).y, compute_placement_pos(h, s).z]); } else { self.last_target = None; }

        if !self.cursor_free && (self.input.action_add || self.input.action_remove || self.input.action_pick) {
            if let Some(ref h) = hit {
                if self.input.action_pick { let pal = self.palette.read().unwrap(); if let Some(&color) = pal.get((h.material - 1) as usize) { self.hotbar_colors[self.selected_slot] = color; drop(pal); self.update_ui(); } } 
                else if self.input.action_remove { let p = h.hit_pos - h.normal * (MIN_VOXEL_SIZE * 0.5); self.chunk_manager.modify_cube(Vec3::new((p.x / s).floor() * s, (p.y / s).floor() * s, (p.z / s).floor() * s), 0, s, &self.device, &self.queue); } 
                else if self.input.action_add { if self.play_mode == PlayMode::Flying || !self.player_collides_at(self.camera.position - Vec3::new(0.0, self.collider.eye_offset, 0.0), &self.collider) { let mat_id = self.get_or_create_material(self.hotbar_colors[self.selected_slot]); self.chunk_manager.modify_cube(compute_placement_pos(h, s), mat_id, s, &self.device, &self.queue); } }
            } else if self.input.action_add {
                let p = self.camera.position + dir * (s * 2.0).clamp(8.0, 64.0); let mat_id = self.get_or_create_material(self.hotbar_colors[self.selected_slot]);
                self.chunk_manager.modify_cube(Vec3::new((p.x / s).floor() * s, (p.y / s).floor() * s, (p.z / s).floor() * s), mat_id, s, &self.device, &self.queue);
            }
            self.input.action_add = false; self.input.action_remove = false; self.input.action_pick = false;
        }

        self.update_camera_buffer(); self.chunk_manager.update(self.camera.position, &self.device); self.update_ui();
    }
    fn render(&mut self) {
        let mut surface_texture = self.surface.get_current_texture();
        if matches!(surface_texture, wgpu::CurrentSurfaceTexture::Outdated | wgpu::CurrentSurfaceTexture::Lost) { self.surface.configure(&self.device, &self.config); surface_texture = self.surface.get_current_texture(); }
        ...
        let mut encoder = self.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });
        {
            let mut render_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("Main Render Pass"), color_attachments: &[Some(wgpu::RenderPassColorAttachment { view: &view, resolve_target: None, ops: wgpu::Operations { load: wgpu::LoadOp::Clear(wgpu::Color { r: self.bg_color[0] as f64, g: self.bg_color[1] as f64, b: self.bg_color[2] as f64, a: 1.0 }), store: wgpu::StoreOp::Store }, depth_slice: None })],
                depth_stencil_attachment: Some(wgpu::RenderPassDepthStencilAttachment { view: &self.depth_texture_view, depth_ops: Some(wgpu::Operations { load: wgpu::LoadOp::Clear(1.0), store: wgpu::StoreOp::Store }), stencil_ops: None }), timestamp_writes: None, occlusion_query_set: None, multiview_mask: None,
            });
            render_pass.set_pipeline(&self.render_pipeline); render_pass.set_bind_group(0, &self.camera_bind_group, &[]);
            for chunk in self.chunk_manager.loaded_chunks.values() { if chunk.num_indices > 0 { render_pass.set_vertex_buffer(0, chunk.vertex_buffer.slice(..)); render_pass.set_index_buffer(chunk.index_buffer.slice(..), wgpu::IndexFormat::Uint32); render_pass.draw_indexed(0..chunk.num_indices, 0, 0..1); } }
        }
        if !self.hide_ui {
            let mut ui_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor { label: Some("UI Render Pass"), color_attachments: &[Some(wgpu::RenderPassColorAttachment { view: &view, resolve_target: None, ops: wgpu::Operations { load: wgpu::LoadOp::Load, store: wgpu::StoreOp::Store }, depth_slice: None })], depth_stencil_attachment: None, timestamp_writes: None, occlusion_query_set: None, multiview_mask: None });
            ui_pass.set_pipeline(&self.ui_pipeline); ui_pass.set_vertex_buffer(0, self.ui_vertex_buffer.slice(..)); ui_pass.draw(0..self.ui_vertices_count, 0..1);
        }
        self.queue.submit(std::iter::once(encoder.finish())); self.queue.present(frame);
    }
}
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        if self.state.is_none() {
            #[allow(unused_mut)]
            let mut window_attributes = Window::default_attributes().with_title("Voxel Studio - 3D PixelArt Engine").with_inner_size(LogicalSize::new(1280.0, 720.0)).with_visible(true);
            #[cfg(target_os = "linux")] { window_attributes = WindowAttributesExtWayland::with_name(window_attributes, "octree_voxels", "octree_voxels"); window_attributes = WindowAttributesExtX11::with_name(window_attributes, "octree_voxels", "octree_voxels"); }
            let window = Arc::new(event_loop.create_window(window_attributes).unwrap());
            let _ = window.set_cursor_grab(winit::window::CursorGrabMode::Locked).or_else(|_| window.set_cursor_grab(winit::window::CursorGrabMode::Confined)); window.set_cursor_visible(false);
            let mut state = pollster::block_on(State::new(Arc::clone(&window)));
            state.render(); self.state = Some(state); self.last_frame = Instant::now(); window.request_redraw();
        }
    }
    fn window_event(&mut self, event_loop: &ActiveEventLoop, _id: WindowId, event: WindowEvent) {
        if let Some(state) = self.state.as_mut() {
            ...
            match event {
                ...
                WindowEvent::DroppedFile(path_buf) => { state.glb_settings.selected_file = Some(path_buf); state.set_menu(ActiveMenu::ImportParams); }
                WindowEvent::CursorMoved { position, .. } => {
                    let ndc_x = (position.x as f32 / state.size.width as f32) * 2.0 - 1.0; let ndc_y = 1.0 - (position.y as f32 / state.size.height as f32) * 2.0; state.cursor_pos = [ndc_x, ndc_y];
                    if state.gimbal_dragging {
                        let dx = ndc_x - state.prev_cursor_pos[0]; let dy = ndc_y - state.prev_cursor_pos[1];
                        if dx.abs() > 0.0005 || dy.abs() > 0.0005 { state.gimbal_drag_moved = true; state.camera.yaw += dx * 3.8; state.camera.pitch = (state.camera.pitch - dy * 3.8).clamp(-1.56, 1.56); state.update_camera_buffer(); state.update_ui(); }
                    } else if state.active_menu == ActiveMenu::Edit {
                        if let Some(channel) = state.active_slider { state.hotbar_colors[state.selected_slot][channel] = ((ndc_x - -0.32) / (0.18 - -0.32)).clamp(0.0, 1.0); state.update_ui(); }
                    }
                    state.prev_cursor_pos = [ndc_x, ndc_y];
                }
                WindowEvent::MouseWheel { delta, .. } => {
                    let step = match delta { MouseScrollDelta::LineDelta(_, y) => if y > 0.0 { -1 } else if y < 0.0 { 1 } else { 0 }, MouseScrollDelta::PixelDelta(pos) => if pos.y > 0.0 { -1 } else if pos.y < 0.0 { 1 } else { 0 } };
                    if step != 0 {
                        if state.input.ctrl_pressed {
                            if state.camera.is_ortho { state.camera.ortho_size = (state.camera.ortho_size * if step < 0 { 0.88 } else { 1.14 }).max(0.0001); state.update_camera_buffer(); state.update_ui(); } 
                            else { state.camera.position += state.camera.forward() * (if step < 0 { 1.5 } else { -1.5 }); state.update_camera_buffer(); state.update_ui(); }
                        } else {
                            state.selected_slot = (state.selected_slot as i32 + step).rem_euclid(10) as usize; state.update_ui();
                        }
                    }
                }
                WindowEvent::KeyboardInput { event: key_event, .. } => {
                    let is_pressed = key_event.state == ElementState::Pressed;
                    if key_event.physical_key == PhysicalKey::Code(KeyCode::ControlLeft) || key_event.physical_key == PhysicalKey::Code(KeyCode::ControlRight) { state.input.ctrl_pressed = is_pressed; }
                    if key_event.physical_key == PhysicalKey::Code(KeyCode::Tab) && is_pressed { let new_free = !state.cursor_free; state.set_cursor_mode(new_free); return; }
                    if (key_event.physical_key == PhysicalKey::Code(KeyCode::NumpadDecimal) || key_event.physical_key == PhysicalKey::Code(KeyCode::Period)) && is_pressed { state.focus_on_scene(); return; }
                    if (key_event.physical_key == PhysicalKey::Code(KeyCode::KeyP) || key_event.physical_key == PhysicalKey::Code(KeyCode::Numpad5)) && is_pressed { state.toggle_projection(); return; }
                    if is_pressed && state.camera.is_ortho {
                        match key_event.physical_key {
                            PhysicalKey::Code(KeyCode::NumpadAdd) | PhysicalKey::Code(KeyCode::Equal) => { state.camera.ortho_size = (state.camera.ortho_size * 0.85).clamp(2.0, 1200.0); state.update_camera_buffer(); state.update_ui(); return; },
                            PhysicalKey::Code(KeyCode::NumpadSubtract) | PhysicalKey::Code(KeyCode::Minus) => { state.camera.ortho_size = (state.camera.ortho_size * 1.15).clamp(2.0, 1200.0); state.update_camera_buffer(); state.update_ui(); return; },
                            _ => {}
                        }
                    }
                    if key_event.physical_key == PhysicalKey::Code(KeyCode::KeyE) && is_pressed { if state.active_menu == ActiveMenu::Edit { state.set_menu(ActiveMenu::None); } else if state.active_menu == ActiveMenu::None { state.set_menu(ActiveMenu::Edit); } return; }
                    if key_event.physical_key == PhysicalKey::Code(KeyCode::F1) && is_pressed { state.hide_ui = !state.hide_ui; state.update_ui(); return; }
                    if key_event.physical_key == PhysicalKey::Code(KeyCode::Escape) && is_pressed { 
                        match state.active_menu { 
                            ...
                            ActiveMenu::Voxelizing => {}, _ => state.set_menu(ActiveMenu::None) 
                        } 
                        return; 
                    }
                    if is_pressed {
                        match key_event.physical_key {
                            PhysicalKey::Code(KeyCode::F2) => state.cycle_world_generator(), PhysicalKey::Code(KeyCode::F5) => { let _ = state.save_game("world_save.json"); }, PhysicalKey::Code(KeyCode::F9) => { let _ = state.load_game("world_save.json"); },
                            PhysicalKey::Code(KeyCode::KeyM) => state.toggle_play_mode(), PhysicalKey::Code(KeyCode::KeyF) => state.scale_voxel_size(false), PhysicalKey::Code(KeyCode::KeyR) => state.scale_voxel_size(true),
                            PhysicalKey::Code(KeyCode::Digit1) => { state.selected_slot = 0; state.update_ui(); }, PhysicalKey::Code(KeyCode::Digit2) => { state.selected_slot = 1; state.update_ui(); }, PhysicalKey::Code(KeyCode::Digit3) => { state.selected_slot = 2; state.update_ui(); }, PhysicalKey::Code(KeyCode::Digit4) => { state.selected_slot = 3; state.update_ui(); }, PhysicalKey::Code(KeyCode::Digit5) => { state.selected_slot = 4; state.update_ui(); }, PhysicalKey::Code(KeyCode::Digit6) => { state.selected_slot = 5; state.update_ui(); }, PhysicalKey::Code(KeyCode::Digit7) => { state.selected_slot = 6; state.update_ui(); }, PhysicalKey::Code(KeyCode::Digit8) => { state.selected_slot = 7; state.update_ui(); }, PhysicalKey::Code(KeyCode::Digit9) => { state.selected_slot = 8; state.update_ui(); }, PhysicalKey::Code(KeyCode::Digit0) => { state.selected_slot = 9; state.update_ui(); },
                    ...
                    if state.active_menu == ActiveMenu::None {
                        match key_event.physical_key {
                            PhysicalKey::Code(KeyCode::KeyW) => state.input.forward = is_pressed, PhysicalKey::Code(KeyCode::KeyS) => state.input.backward = is_pressed, PhysicalKey::Code(KeyCode::KeyA) => state.input.left = is_pressed, PhysicalKey::Code(KeyCode::KeyD) => state.input.right = is_pressed, PhysicalKey::Code(KeyCode::Space) => state.input.up = is_pressed, PhysicalKey::Code(KeyCode::ShiftLeft) => state.input.down = is_pressed,
                            _ => {}
                        }
                    }
                }
                    ...
                    if button == MouseButton::Left {
                        if element_state == ElementState::Pressed {
                            if state.cursor_free || state.active_menu != ActiveMenu::None {
                                ...
                                if ((mx - GIZMO_CENTER_X) * aspect).powi(2) + (my - GIZMO_CENTER_Y).powi(2) <= (GIZMO_RADIUS + 0.02).powi(2) { state.gimbal_dragging = true; state.gimbal_drag_moved = false; return; }
                            }
                        } else if element_state == ElementState::Released && state.gimbal_dragging {
                            state.gimbal_dragging = false;
                            if !state.gimbal_drag_moved {
                                let right = state.camera.right(); let up = state.camera.up(); let mut sorted_axes = get_gizmo_axes();
                                ...
                                for ax in sorted_axes { if ((mx - (GIZMO_CENTER_X + (ax.dir.dot(right) * GIZMO_RADIUS) / aspect)) * aspect).powi(2) + (my - (GIZMO_CENTER_Y + (ax.dir.dot(up) * GIZMO_RADIUS))).powi(2) <= 0.001225 { state.camera.yaw = ax.yaw; state.camera.pitch = ax.pitch; state.camera.is_ortho = true; state.update_camera_buffer(); state.update_ui(); return; } }
                            }
                            return;
                        }
                    }
                    match state.active_menu {
                        ActiveMenu::Edit => {
                            if button == MouseButton::Left && element_state == ElementState::Pressed {
                                ...
                                if mx >= -0.34 && mx <= 0.20 { state.active_slider = if my >= 0.30 && my <= 0.38 { Some(0) } else if my >= 0.23 && my <= 0.31 { Some(1) } else if my >= 0.16 && my <= 0.24 { Some(2) } else { None }; if let Some(channel) = state.active_slider { state.hotbar_colors[state.selected_slot][channel] = ((mx - -0.32) / (0.18 - -0.32)).clamp(0.0, 1.0); state.update_ui(); return; } }
                                let pw_w = 0.076; let pw_gap = 0.012; let pw_tot = 10.0 * pw_w + 9.0 * pw_gap; let pw_start_x = -pw_tot / 2.0;
                                ...
                                if mx >= -0.40 && mx <= -0.22 && my >= -0.10 && my <= -0.02 { state.scale_voxel_size(false); return; } if mx >= 0.22 && mx <= 0.40 && my >= -0.10 && my <= -0.02 { state.scale_voxel_size(true); return; } if mx >= -0.22 && mx <= 0.22 && my >= -0.25 && my <= -0.17 { state.set_menu(ActiveMenu::None); return; }
                        ...
                        ActiveMenu::ImportParams => {
                            if button == MouseButton::Left && element_state == ElementState::Pressed {
                                ...
                                if my >= 0.19 && my <= 0.27 { if mx >= wx0 + 0.04 && mx <= wx0 + 0.14 { state.glb_settings.target_height = (state.glb_settings.target_height - 4.0).max(4.0); state.update_ui(); return; } if mx >= wx1 - 0.14 && mx <= wx1 - 0.04 { state.glb_settings.target_height = (state.glb_settings.target_height + 4.0).min(64.0); state.update_ui(); return; } }
                                if my >= -0.01 && my <= 0.07 { if mx >= wx0 + 0.04 && mx <= wx0 + 0.14 { state.glb_settings.voxel_size *= 0.5; state.update_ui(); return; } if mx >= wx1 - 0.14 && mx <= wx1 - 0.04 { state.glb_settings.voxel_size *= 2.0; state.update_ui(); return; } }
                                if my >= -0.21 && my <= -0.13 { if mx >= wx0 + 0.04 && mx <= wx0 + 0.14 { state.glb_settings.palette_size = (state.glb_settings.palette_size / 2).max(2); state.update_ui(); return; } if mx >= wx1 - 0.14 && mx <= wx1 - 0.04 { state.glb_settings.palette_size = (state.glb_settings.palette_size * 2).min(8192); state.update_ui(); return; } }
                                if mx >= wx0 + 0.04 && mx <= wx1 - 0.04 && my >= -0.39 && my <= -0.31 { state.glb_settings.place_at_aim = !state.glb_settings.place_at_aim; state.update_ui(); return; }
                                if mx >= wx0 + 0.04 && mx <= wx1 - 0.04 && my >= -0.54 && my <= -0.44 { state.start_nonblocking_voxelization(); return; }
                                if mx >= wx0 + 0.04 && mx <= wx1 - 0.04 && my >= -0.66 && my <= -0.56 { state.set_menu(ActiveMenu::Pause); return; }
                            }
                        }
                        ActiveMenu::Voxelizing => {}
                        ActiveMenu::None => {
                            if element_state == ElementState::Pressed && !state.cursor_free {
                                match button { MouseButton::Left => state.input.action_add = true, MouseButton::Right => state.input.action_remove = true, MouseButton::Middle => state.input.action_pick = true, _ => {} }
                            }
                        }
                    }
                }
        ...
        if let Some(state) = self.state.as_mut() {
            if state.active_menu == ActiveMenu::None && !state.cursor_free && !state.gimbal_dragging {
                if let DeviceEvent::MouseMotion { delta } = event {
                    state.camera.yaw += (delta.0 as f32) * 0.002; state.camera.pitch -= (delta.1 as f32) * 0.002; state.camera.pitch = state.camera.pitch.clamp(-1.56, 1.56);
                }
            }
        }
    }
